<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Barre de recherche -->
            <div class="mb-8">
                <form method="GET" action="{{ route('bibliotheque') }}" class="flex gap-4">
                    <div class="flex-1">
                        <input type="text"
                               name="search"
                               value="{{ $searchQuery }}"
                               placeholder="Rechercher un livre par titre, auteur, catégorie ou ISBN..."
                    ...
                    @foreach($livres as $livre)
                        <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-200">
                            ...
                            <div class="p-4">
                                ...
                                @if(isset($livre['api_data']['description']) && $livre['api_data']['description'])
                                    <p class="text-sm text-gray-700 mb-3 line-clamp-3">
                                        {{ Str::limit($livre['api_data']['description'], 120) }}
                                    </p>
                                @endif
                                ...
                                <div class="flex gap-2">
                                    @if($livre['chemin_fichier'])
                                        <a href="{{ Storage::url($livre['chemin_fichier']) }}"
                                           target="_blank"
                                           class="flex-1 bg-green-600 text-white text-sm px-3 py-2 rounded hover:bg-green-700 transition duration-200 text-center">
                                            Télécharger
                                        </a>
                                    @endif
                                    ...
                                    @if(isset($livre['api_data']['info_link']) && $livre['api_data']['info_link'])
                                        <a href="{{ $livre['api_data']['info_link'] }}"
                                           target="_blank"
                                           class="flex-1 bg-blue-600 text-white text-sm px-3 py-2 rounded hover:bg-blue-700 transition duration-200 text-center">
                                            Google Books
                                        </a>
                                    @endif
                                </div>
