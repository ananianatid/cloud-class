...
class CalendarCoursRelationManager extends RelationManager
{
    protected static string $relationship = 'cours';

    protected static ?string $title = 'Vue Calendrier des Cours';
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('duree')
                    ...
                    ->getStateUsing(function ($record) {
                        $debut = \Carbon\Carbon::parse($record->debut);
                        $fin = \Carbon\Carbon::parse($record->fin);
                        $duree = $debut->diffInMinutes($fin);
                        $heures = intval($duree / 60);
                        $minutes = $duree % 60;
                        return $heures . 'h' . ($minutes > 0 ? str_pad($minutes, 2, '0', STR_PAD_LEFT) : '');
                    })
                    ->badge()
                    ->color('secondary')
                    ->width('80px'),
            ...
            ->headerActions([
                ...
                Tables\Actions\BulkAction::make('create_weekly_schedule')
                    ->label('Créer un planning hebdomadaire')
